...
export default function HomeScreen() {
  ...
  const stopRecording = async () => {
   ...
   const sendAudioToWhisper = async (uri: string) => {
     try {
       ...
       console.log("File part in FormData:", formData._parts[0][1]);
          ...
          messages: [
            {
              role: "system",
              content:
                "You are Catherine, a friendly AI assistant who responds naturally and referes to yourself as Catherine when asked for your name. You are a helpful assistant who can answer questions and help with tasks. You must always respond in English, no matter the input language,and provide helpful, clear answers",
            },
            {
              role: "user",
              content: text,
  ...
  const uploadImageToImgur = async (imageUri: string) => {
    try {
      ...
      formData.append("image", {
        uri: imageUri, // Path to the image on your device
        type: "image/jpeg", // Ensure correct type
        name: "photo.jpg", // Provide a name for the uploaded file
      });
  
      const response = await axios.post("https://api.imgur.com/3/upload", formData, {
        headers: {
          Authorization: "Client-ID xxxxxxxxxxxxxxx", // Replace with your Imgur Client ID
          "Content-Type": "multipart/form-data",
        },
      });
      ...
      console.error("Error uploading image to Imgur:", error.response?.data || error);
  ...
  const sendImageToChatGPT = async (imageUri: string) => {
    try {
      ...
      // Step 2: Send the image URL to ChatGPT
          ...
          messages: [
            {
              role: "system",
              content:
                "You are Catherine, a friendly AI assistant. Analyze the provided image and describe it.",
            },
            {
              role: "user",
              content: `Describe this image: ${imageUrl}`,
            },
          ],
        },
        {
  ...
  {loading ? (
    ...
    <>
      {!AIResponse ? (
        // When AI response is not available, show the camera icon
        <TouchableOpacity
          style={{
            width: scale(110),
            height: scale(110),
          ...
          onPress={openCamera} // Trigger camera on press
        >
          <FontAwesome name="camera" size={scale(50)} color="#2b3356" />
        </TouchableOpacity>
      ) : (
        // When AI response is available, show the AI speaking animation
        <View>
          <LottieView
            ref={lottieRef}
            source={require("@/assets/animations/ai-speaking.json")}
